...
export default class Bot 
{
  ...
  */
    ...
    var postNum = 17; // Specify the number of recent posts to compare from the logged in user's feed.
    var bskyFeedAwait = await this.userAgent.app.bsky.feed.getAuthorFeed({actor: "gooooooosens.bsky.social", limit: postNum,}); // Get a defined number + 2 of most recent posts from the logged in user's feed.
      ...
      if (card != "None" && urls[0] == "None")
      {
        ...
        var cardBuffer = Buffer.from(cardResponse.data, "utf-8");
          if (cardBuffer.length > 1000000)
          {
            ...
            cardResponse = await axios.get("https://files.mastodon.social/cache/accounts/avatars/111/840/105/523/898/246/original/660e2d5a4e49d9f7.png", { responseType: 'arraybuffer'}); 
