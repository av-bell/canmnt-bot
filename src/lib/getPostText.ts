...
{
	const limitVal = 27; // The number of posts to get from Mastodon.
	...
	for (let i = 0; i < limitVal; i++) // Iterate over all the posts we collected using the Mastodon API. 
	{
		...
		if (contentString.includes("@Kamille_brick") || contentString.includes("@GraemeNichols") || contentString.includes("@RicFlairNatrBoy")
		{
			contentString = "";
		}
