...
export default async function getPostText() 
{
	...
	for (let i = 0; i < limitVal; i++) // Iterate over all the posts we collected using the Mastodon API. 
	{
		...
		if (contentString.includes("@Kamille_brick") || contentString.includes("further accentuates") || contentString.includes("salt of the earth") || contentString.includes("poors") || contentString.includes("@RicFlairNatrBoy") || contentString.includes("better than you") || contentString.includes("MJF") || contentString.includes("@tompestock") || contentString.includes("@DraftKings"))
		{
			contentString = "";
		}
		   ...
		   )
		{
			...
			contentString = contentString.replace("@Twins","Minnesota Twins");
			contentString = contentString.replace("@AntDavis23","Anthony Davis");
